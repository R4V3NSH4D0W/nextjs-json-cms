...
export default async function DashboardHomePage() {
      ...
      <div className="flex min-h-[80vh] flex-col items-center justify-center gap-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            You need to select a project workspace to manage content, media, and settings.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-4xl mt-4">
          {projects.map((p) => (
            ...
            >
              <span className="text-xs font-bold uppercase tracking-widest text-primary mb-2 opacity-0 group-hover:opacity-100 transition-opacity">Launch Workspace</span>
              <h3 className="font-bold text-lg">{p.name}</h3>
              <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{p.description || "Project instance"}</p>
            </Link>
          ))}
          {projects.length === 0 && (
            <div className="col-span-full py-12 text-muted-foreground">
              No projects found. Contact an administrator to be added to a project.
            </div>
          )}
        </div>
      </div>
  ...
  const [cmsPageCount, cmsLayoutCount] = await Promise.all([
    fetchCount(`/api/v1/admin/projects/${currentProject.slug}/cms/pages`, cookieHeader),
    fetchCount(`/api/v1/admin/projects/${currentProject.slug}/cms/layouts`, cookieHeader),
  ]);
    ...
    <div className="flex flex-col gap-8 pb-12">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground text-xs flex items-center gap-1">
            Active: <code className="bg-muted px-1 rounded">{currentProject.slug}</code>
          </span>
        </div>
        ...
        <p className="text-muted-foreground text-sm max-w-2xl leading-relaxed">
          Manage your project&apos;s digital presence, layouts, and public content API.
          Everything modified here is isolated to the <strong>{currentProject.name}</strong> environment.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="md:col-span-2 shadow-sm border-border/50 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-4 rounded-lg bg-muted/40 p-4 border border-border/30">
                ...
                <div>
                  <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-tight">Active Pages</p>
                  <p className="text-xl font-bold tabular-nums">{cmsPageCount}</p>
                </div>
              </div>
              <div className="flex items-center gap-4 rounded-lg bg-muted/40 p-4 border border-border/30">
                <div className="rounded-full bg-primary/10 p-2 text-primary">
                ...
                <div>
                  <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-tight">UI Layouts</p>
                  <p className="text-xl font-bold tabular-nums">{cmsLayoutCount}</p>
                </div>
              </div>
            </div>
          </CardContent>
          <CardFooter className="pt-2 border-t border-border/20 text-[10px] flex justify-between">
            <span className="text-muted-foreground flex items-center gap-1">
              <Settings className="size-3" /> Status: {currentProject.status || 'Active'}
            </span>
            <span className="text-muted-foreground flex items-center gap-1">
              API Base: <code className="bg-muted px-1 rounded">/api/v1/projects/{currentProject.slug}</code>
            </span>
          </CardFooter>
        </Card>
        ...
        <Card className="md:col-span-1 shadow-sm border-border/50 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent className="flex flex-col gap-2">
            <Button asChild className="w-full justify-start gap-2 h-10 shadow-sm">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="shadow-sm border-border/50 bg-card/60">
          <CardHeader className="pb-3 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Digital Assets</CardHeader>
          <CardContent className="flex flex-wrap gap-2">
            <Button asChild size="sm" variant="secondary" className="h-9 px-4 gap-2">
        ...
        <Card className="shadow-sm border-border/50 bg-card/60">
          <CardHeader className="pb-3 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Information</CardHeader>
          <CardContent className="space-y-2">
            <p className="text-xs text-muted-foreground italic leading-relaxed">
              Modifications made in this workspace are instantly available to the public content API under the project slug <strong>{currentProject.slug}</strong>.
            </p>
          </CardContent>
        </Card>
      </div>
